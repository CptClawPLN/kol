...
function main() {
	...
	this.drinkPotion = function (type) {
		...
		case 2:
		case 4:
			if (timerLastDrink[type] && (tNow - timerLastDrink[type] < 2000)) { // small delay for juvs just to prevent using more at once
				return false;
			}
	...
	//Test to get state id
    this.getStateActive = function () {
        var i, unit, id;
        
        
        for (i = 243; i < 250; i++) {
            if (me.getState(i) == true) {
                unit = i;
                id = "false";
            }
        }
	...
	this.keyEvent = function (key) {
		...
		case 110: // decimal point
			//say("/fps");
			//print(this.getStateActive());
			//FileTools.writeText("logs/itemstatID.txt", NTIP.ParseLineInt(-1, 1) + "\n");
