...
var Precast = new function () {
	...
	{
		if (!force && me.getState(30)) 
		{
			return true;
		}

		if (me.gametype === 0 || me.classid === ( 2 || 3 || 4 || 5 || 6 ) || me.inTown) 
		{
			return false;
		}
		...
		if (this.checkMEMORY()) 
		{
			...
			this.weaponSwitch(this.haveMEMORY);
			...
			delay(250);
			this.ESDuration = (80 + (me.getSkill(58, 0) * 60)) * 1000;
			this.ESTick = getTickCount();
			delay(10);
	...
	this.getBetterSlot = function (skillId) {
		...
		case 52: // Enchant
			...
			break;
		case 47: // Fireball - Check that we are back on our main skill swap.
		case 56: // Meteor - Double Check
			classid = 1;
			skillTab = 8;
			
			break;
		case 59: // Blizzard - Check that we are back on our main skill swap.
		case 55: // Glacial Spike - Double Check
			classid = 1;
			skillTab = 10;
			
			break;
		case 49: // Lightning - Check that we are back on our main skill swap.
		case 53: // Chain Lightning - Double Check
			classid = 1;
			skillTab = 9;
	...
	this.checkMEMORY = function () {
		...
		if (item) {
			do {
				if (item.getPrefix(20588)) { // Memory
					switch (item.bodylocation) {
					case 4:
					case 5:
						this.haveMEMORY = me.weaponswitch;

						return true;
					case 11:
					case 12:
						this.haveMEMORY = me.weaponswitch ^ 1;
